...
## I. Secrets / credential exposure review (Phase 8) — REVIEW ONLY, no exposure found

**Result: NO exposed secrets found in the repo, git history, CI, deploy config,
host config, systemd units, or logs. NO rotation is required by this review.**
Full detail: `deploy/SECRETS-EXPOSURE-REVIEW.md`.

Verified good hygiene (no action needed): only `*.env.example` placeholders are
tracked; `.gitignore` excludes real `.env`; git history clean; production
JWT/DB/AWS/Redis secrets are set + strong and NOT the dev fallback; the prod
fail-fast guard blocks boot on weak/missing JWT secrets; host `.env` files are
600/root; monitor `alert.conf` and backup cred files are 600/root; backups are
age-encrypted off-host; no secrets echoed in logs.

**I1 — 🟢 LOW / OPTIONAL / operator-only (routine hygiene, NOT triggered by any
finding):** consider a calendar-based rotation policy for long-lived secrets
(`JWT_SECRET`/`JWT_REFRESH_SECRET`, `DATABASE_PASSWORD`, AWS keys, `REDIS_PASSWORD`,
Google OAuth client secret). This is discretionary; nothing in Phase 8 indicates
any of these has leaked.
- **Verification after any rotation:** update the env, `docker compose up -d`
  the affected service, confirm `handla_api` boots (fail-fast guard passes) and
  login/refresh still work; for AWS, confirm S3 upload/download still succeed.
- **Security warning:** rotate JWT secrets during a low-traffic window (existing
  sessions/refresh tokens signed with the old secret become invalid).

---

## J. Final code / dependency review (Phase 9) — REVIEW ONLY, web is clean

Full detail: `deploy/CODE-DEPENDENCY-REVIEW.md`.

**Web production is clean:** backend build ✅ + 1275/1275 tests; frontend lint ✅
+ 40/40 tests; **npm audit = 0 vulns** for both backend and frontend (prod and
incl-dev). Next.js 15.5.23 is past the CVE-2025 middleware-bypass fix. All core
security controls present in code (JWT/roles/ownership guards, throttler, helmet,
CORS, whitelist ValidationPipe, public-doc token, socket JWT, prod secret
fail-fast). No injectable raw SQL. No unrelated upgrades introduced.

**J1 — 🟢 LOW / OPTIONAL / operator-only (OUT OF PRODUCTION WEB SCOPE):**
`handla-mobile` (Expo/React-Native, NOT deployed to the production VPS) has 20
`npm audit` findings (9 high, 11 moderate), all in the **build-time toolchain**
(`image-size`, `postcss`) — they run only during local bundling and do not
affect the production website or API. If/when the mobile app is next worked on,
bump the Expo/React-Native toolchain to clear them. Deliberately NOT changed
here (out of scope + would require breaking toolchain upgrades).

## K. Authenticated authorization walkthrough (Phase 10) — 🟡 MEDIUM / operator-only (NOT LIVE VERIFIED)

Full detail: `deploy/PRODUCTION-VALIDATION.md`.

Every Phase 10 control that can be exercised **without** a login was live-verified
and PASSED (headers, TLS, unauthenticated 401s, input validation, forged
public-doc tokens → 404, rate limiting, CORS lock, service isolation, minimal
public ports, Socket.IO unauthenticated disconnect, private S3 bucket, traversal
→ 404). Production stayed healthy before and after.

**K1 — 🟡 What still needs an operator to confirm end-to-end (requires valid
production credentials — not available to the automated run, so marked
NOT LIVE VERIFIED rather than guessed):**

- **Role separation** — sign in as an `ADMIN` and as a `CLIENT` and confirm each
  role can reach only its intended endpoints (RolesGuard / `@Roles`).
- **Cross-tenant / cross-account isolation** — as an authenticated CLIENT,
  attempt to read another account's invoice / quotation / conversation by ID and
  confirm **403/404**, never another party's data (OwnershipGuard).
- **Ownership on real records** — confirm update/delete of a record you do not
  own is rejected.
- **Realtime per-record authZ** — as an authenticated non-participant, attempt to
  join/read a conversation you are not part of and confirm rejection.

**Why this is only MEDIUM, not blocking:** the controls are all present in code
and covered by unit tests (RolesGuard, OwnershipGuard, per-record checks in the
chat gateway, 1275/1275 backend tests passing). This item is a *live confirmation*
of already-tested logic, not a discovered gap. Recommended as a routine
post-deploy smoke check whenever the operator next logs in with real accounts.
