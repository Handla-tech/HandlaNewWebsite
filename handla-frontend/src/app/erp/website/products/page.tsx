...
/**
 * ERP — Website Products Management (/erp/website/products)
 * ADMIN-only: full CRUD on public-facing website products / solutions.
 */
...
import {
  Package, Plus, Pencil, Trash2, X, Loader2, AlertCircle,
...
function ProductModal({
  ...
  const {
    ...
    defaultValues: {
      ...
      price:       initial?.price       ?? '',
      featuresCsv: (initial?.features ?? []).join(', '),
  ...
  const onSubmit = async (data: FormData) => {
    setSubmitError(null);
    const features = (data.featuresCsv ?? '').split(',').map((s) => s.trim()).filter(Boolean);
    const payload = {
      name:        data.name,
    ...
    try {
      if (isEdit) await websiteProductApi.update(initial!.id, payload);
      else await websiteProductApi.create(payload);
      onSaved();
    } catch (err: unknown) {
      const msg =
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
      ...
      <div className="p-4">
        ...
        {p.price && <p className="mt-0.5 text-[11px] text-[#fbbf24]">{p.price}</p>}
        <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-[#888]">{p.tagline || p.description}</p>
      </div>
    </motion.div>
  );
}
...
export default function WebsiteProductsPage() {
  ...
  const [editTarget, setEditTarget] = useState<WebsiteProduct | null>(null);
  const editTargetRef = useRef<WebsiteProduct | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<WebsiteProduct | null>(null);
  const [successMsg, setSuccessMsg] = useState<string | null>(null);

  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: [QUERY_KEY, page],
    queryFn: async () => {
      const res = await websiteProductApi.getAll({ page, limit: PAGE_SIZE });
      const payload = (res.data as { data?: { products?: WebsiteProduct[]; total?: number } })?.data;
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => websiteProductApi.remove(id),
    onSuccess: () => {
  ...
  const handleOpenEdit = (p: WebsiteProduct) => { editTargetRef.current = p; setEditTarget(p); setModalOpen(true); };
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
        ...
        {!isLoading && !isError && filtered.length > 0 && (
          <AnimatePresence mode="popLayout">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filtered.map((p) => (
                <ProductRow key={p.id} p={p} onEdit={() => handleOpenEdit(p)} onDelete={() => setDeleteTarget(p)} />
              ))}
            </div>
          </AnimatePresence>
        )}
      ...
      <AnimatePresence>
        {modalOpen && (
          <ProductModal key="modal" initial={editTarget}
            onClose={() => { setModalOpen(false); setEditTarget(null); }} onSaved={handleModalSaved} />
        )}
        {deleteTarget && (
          <DeleteDialog key="delete" name={deleteTarget.name}
