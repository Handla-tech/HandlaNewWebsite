  ...
  if (projects.length === 0) {
    return (
      <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl p-10 flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-gray-500 text-sm">{t('erp.clients.detail.noProjects')}</p>
      </div>
    );
  }
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-3 mt-2 text-xs text-gray-500">
                  ...
                  {project.endDate && (
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {t('erp.clients.detail.due')} {formatDate(project.endDate)}
                    </span>
                  )}
                  {project.owner && (
                    <span className="flex items-center gap-1">
              ...
              >
                ...
                {t(`erp.projectStatus.${project.status}`)}
              </span>
            </div>
          </Link>
        );
  ...
  if (contracts.length === 0) {
    return (
      <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl p-10 flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-gray-500 text-sm">{t('erp.clients.detail.noContracts')}</p>
      </div>
    );
  }
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-3 mt-2 text-xs text-gray-500">
                  <span className="flex items-center gap-1">
                    <Calendar className="w-3 h-3" /> {t('erp.clients.detail.created')} {formatDate(contract.createdAt)}
                  </span>
                  {contract.sentAt && (
                    <span className="flex items-center gap-1">
                      <Send className="w-3 h-3" /> {t('erp.clients.detail.sent')} {formatDate(contract.sentAt)}
                    </span>
                  )}
                  {contract.signedAt && (
                    <span className="flex items-center gap-1">
                      <PenLine className="w-3 h-3" /> {t('erp.clients.detail.signed')} {formatDate(contract.signedAt)}
                    </span>
                  )}
                  {contract.owner && (
                    <span className="flex items-center gap-1">
  ...
  if (invoices.length === 0) {
    return (
      <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl p-10 flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-gray-500 text-sm">{t('erp.clients.detail.noInvoices')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: t('erp.clients.detail.paid'),    amount: totalPaid,    cls: 'text-emerald-400' },
          { label: t('erp.clients.detail.unpaid'),  amount: totalUnpaid,  cls: 'text-yellow-400'  },
          { label: t('erp.clients.detail.overdue'), amount: totalOverdue, cls: 'text-red-400'     },
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-3 mt-1.5 text-xs text-gray-500">
                    <span className="flex items-center gap-1">
                      <Calendar className="w-3 h-3" /> {t('erp.clients.detail.issued')} {formatDate(invoice.createdAt)}
                    </span>
                    {invoice.dueDate && (
                      <span className="flex items-center gap-1">
                        <Clock className="w-3 h-3" /> {t('erp.clients.detail.due')} {formatDate(invoice.dueDate)}
                      </span>
                    )}
                    {invoice.paidAt && (
                      <span className="flex items-center gap-1">
                        <CheckCircle2 className="w-3 h-3 text-emerald-400" /> {t('erp.clients.detail.paid')} {formatDate(invoice.paidAt)}
                      </span>
                    )}
                  </div>
                </div>
                <div className="text-right shrink-0">
                  ...
                  {invoice.taxRate > 0 && (
                    <p className="text-xs text-gray-600 mt-0.5">{t('erp.clients.detail.inclTax', { rate: invoice.taxRate })}</p>
                  )}
                </div>
              </div>
            </Link>
  ...
  if (isError || !client) {
    return (
      <div className="flex flex-col items-center gap-4 py-20 text-center">
        ...
        <p className="text-gray-400">{t('erp.clients.detail.notFound')}</p>
        <button
        ...
        >
          {t('erp.clients.detail.backToClients')}
        </button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400">
        <Link href="/erp/clients" className="flex items-center gap-1 hover:text-white transition-colors">
          <ArrowLeft className="w-4 h-4" /> {t('erp.clients.title')}
        </Link>
        <span>/</span>
      ...
      <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-4 mt-2 text-xs text-gray-500">
            ...
            <span className="flex items-center gap-1">
              <User className="w-3.5 h-3.5" /> {t('erp.clients.detail.owner')}: {ownerName}
            </span>
            <span className="flex items-center gap-1">
              <Calendar className="w-3.5 h-3.5" /> {t('erp.clients.detail.created')} {formatDate(client.createdAt)}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl p-5">
            <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
              <StickyNote className="w-4 h-4 text-[#fbbf24]" /> {t('erp.clients.detail.notesTitle')}
            </h3>
            {client.notes ? (
              <p className="text-sm text-gray-300 whitespace-pre-wrap">{client.notes}</p>
            ) : (
              <p className="text-sm text-gray-500 italic">{t('erp.clients.detail.noNotes')}</p>
            )}
          </div>

          {/* Metadata */}
          <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl p-5">
            <h3 className="text-sm font-semibold text-white mb-3">{t('erp.clients.detail.details')}</h3>
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between">
                <dt className="text-gray-500">{t('erp.ui.status')}</dt>
                <dd className={cn('font-medium', CLIENT_STATUS_BADGE[client.status as ClientStatus].split(' ').pop())}>{t(`erp.clients.status.${client.status}`)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">{t('erp.ui.company')}</dt>
                <dd className="text-gray-300">{client.company ?? '—'}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">{t('erp.clients.detail.owner')}</dt>
                <dd className="text-gray-300">{ownerName}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">{t('erp.ui.createdAt')}</dt>
                <dd className="text-gray-300">{formatDate(client.createdAt)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">{t('erp.ui.updatedAt')}</dt>
