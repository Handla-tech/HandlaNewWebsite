...
/**
 * Public Quotation View — accessed via the shareable link on a quotation.
 *
 *   URL pattern: /quotation/public/:token
 *   Endpoints:   GET  /erp/quotations/public/:token          (Nest @Public())
 *                POST /erp/quotations/public/:token/accept    (Nest @Public())
 *                POST /erp/quotations/public/:token/reject    (Nest @Public())
 *
 * Not inside /erp — middleware never gates it and the backend routes need no JWT.
 * The endpoint returns a sanitized projection (no internal owner metadata).
 *
...
export default function PublicQuotationPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'Quotation not found or link expired.');
    } finally {
      setLoading(false);
    }
  }
