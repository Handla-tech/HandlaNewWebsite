...
/**
 * Shared public invoice renderer.
 *
 * Consumed by BOTH public invoice routes:
 *   - /invoice/public/token/:token   (INFO-01 canonical capability link)
 *   - /invoice/public/:id            (LEGACY raw-id link, transitional)
 *
 * The route page owns *how* the data is loaded (which passes the correct
 * token/id endpoint via the `load` prop); this component owns rendering plus
 * consistent invalid / expired / revoked error UX. No internal error data is
 * shown to the public — only a short human message.
 */
...
/**
 * Map the backend status/error into a short, non-leaking public message.
 * 410 Gone → revoked or expired capability link. 404 → invalid/unknown.
 */
function publicErrorMessage(status?: number, serverMsg?: string): string {
  ...
  // Never surface internal stack/error detail; fall back to a generic message.
  return serverMsg && status && status < 500 ? serverMsg : 'This invoice could not be loaded.';
}

export default function InvoicePublicView({ load }: { load: () => Promise<PublicInvoice> }) {
